...
def create_structure(treatment_info, batch_quantity):
    ...
    for i, info in enumerate(treatment_info):
        ...
        output_structure[treatment_dir_name] = {}
        output_structure["static"][treatment_char] = 0

        for batch_num in range(batch_quantity):
            output_structure[treatment_dir_name][f"Batch {batch_num + 1}"] = 0
            # output_structure["static"][treatment_char][f"Batch {batch_num + 1}"] = 0


    return output_structure


def CreateProject(project_dir, treatment_info, day_num=1, batch_quantity = 3):
    """
    Create a project structure at base_path from structure.
    """
    logger.info(f"Creating project structure at {project_dir}, day {day_num}")

    batch_dir = os.path.join(project_dir, f"Day {day_num}")

    project_structure = create_structure(treatment_info, batch_quantity)
